...
const ContactScreen = ({ pageRefs }) => {
  ...
  const onSubmit = (evt) => {
    ...
    if (name) {
      ...
      fetch(
        production ? "/contact" : "http://localhost:3000/contact",
        requestOptions
      ).then((response) => {
  ...
  return (
    ...
    >
      <div className="container">
        ...
        {emailError && (
          <h4>
            Something went wrong sending the email, please wait a moment and try
            again.
          </h4>
        )}
        {!emailSent && (
              ...
              <div className="email-flex-column">
                ...
                {!emailValid && (
                  <div className="email-invalid">
                    Please enter your email address, or leave this field blank.
                  </div>
                )}
              </div>
            </div>
        ...
        {emailSent && (
          <h3>
            Thank you for reaching out! I will be in contact with you shortly!
          </h3>
        )}
      </div>
    </section>
